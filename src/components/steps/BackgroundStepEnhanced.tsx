...
const BackgroundStepContent: React.FC = () => {
  ...
  // Pagination logic
  const totalPages = Math.ceil(reactBitsBackgrounds.length / ITEMS_PER_PAGE);
  const paginatedBackgrounds = useMemo(() => {
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    const endIndex = startIndex + ITEMS_PER_PAGE;
    return reactBitsBackgrounds.slice(startIndex, endIndex);
  }, [currentPage]);
            ...
            <div className="relative p-4 flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm text-gray-300">
                  Live animated previews • {reactBitsBackgrounds.length} backgrounds available • Page {currentPage} of {totalPages}
                </p>
              </div>
            </div>
          </div>
...
const BackgroundStepEnhanced: React.FC = () => {
  return (
    <ErrorBoundary
      fallback={
        <StepErrorFallback
...
export default BackgroundStepEnhanced;
